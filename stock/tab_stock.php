...
<head>
    ...
    <meta content="Mediasoft" name="Mediasoft" />
                    ...
                    <div class="collapse navbar-collapse" id="navbarSupportedContent">
                        <div class="navigation d-flex">
                            <ul class="navbar-nav nav-left">
								...
								<li class="nav-item">
                                    <a class="nav-link   id="navbarDropdown" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">Sortie de stock  </a> <a class="nav-link   id="navbarDropdown" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"> &nbsp;</a> 
                    ...
                    <div class="sidebar-nav scrollbar scroll_light">
                        <ul class="metismenu " id="sidebarNav">
                            <li class="nav-static-title"><? echo 'Profil:'.$_SESSION["Profileur"].'-'.$_SESSION["id_user"];?></li>
							 ...
							 <li><a href="tab_evol_stock.php" aria-expanded="false"><i class="nav-icon ti ti-comment"></i><span class="nav-title">Evolution du stock</span></a>   
                            </li>
                            <li>
                                    ...
                                    <div class="card-body">
                                        <div class="media">
                                            <div class="media-body">
                                                <div class="form-row">		
												<div class="col-sm-4">
													<button type="button" class="btn btn-info" data-toggle="modal" data-target="#myenc">Sortie de Stock</button>
												</div>
                                    ...
                                    <div class="card-body">
                                        <div class="table table-striped mb-0">
                                            <table id="example" style="width:100%" class="display compact table table-striped table-bordered">
                                                ...
                                                <tbody>
                                                    <?				
													$sql = "SELECT md_produit.id_produit,md_produit.designation,md_lieu.nom_lieu,md_stock.date as dater,md_stock.mesure_cm as mesure,
														md_stock.quantite_kg as quant
														FROM public.md_produit,public.md_lieu,md_stock
														WHERE md_produit.id_produit= md_lieu.id_produit 
														AND md_stock.id_produit=md_produit.id_produit
														AND md_stock.id_lieu=md_lieu.id_lieu														
														ORDER BY md_stock.date DESC ";			
                            ...
                            <div class="modal-dialog modal-dialog-centered" role="document">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h5 class="modal-title" id="verticalCenterTitle"> Sortie de stock </h5>
                                    ...
                                    <div class="modal-body">
										...
										<? $sql = "SELECT id_produit,designation FROM public.md_produit ORDER BY id_produit ASC";			
											...
											<div class="col-12">
												...
												<select  class="form-control form-control-sm" name="id_produit" id="id_produit" onchange="charger_lieu()">
												...
												<? $sql="SELECT DISTINCT id_lieu,nom_lieu from md_lieu ";
													$result= $conn->prepare($sql);
													$result->execute();
												?>
												<select class="form-control form-control-sm" name="id_lieu"	id="id_lieu">
													<option>Choisir un lieu de stockage</option>
													<? while ($row1 = $result->fetch(PDO::FETCH_ASSOC ))
													{?>
													<option value="<? echo $row1['id_lieu'];?> "><?echo $row1['nom_lieu']; ?></option> 
													<? } ?>
												</select>
												...
												<input style="int" class="form-control"  name="ltrcm" placeholder="Litres par cm" disabled />
												</div>
												<div class="form-group">
												<label class="control-label">Kg par cm </label>												
												<input style="int" class="form-control"   name="kcm" placeholder="Kg par cm " disabled />
												</div>
												<div class="form-group">
												<label class="control-label">Mesure centimètre </label>												
												<input style="int" class="form-control"  value="0" name="lcm" placeholder="Mesure centimètre" />
												</div>
												<div class="form-group">
												<label class="control-label">Quantité kg par lieu de stockage</label>												
												<input style="int" class="form-control"  value="0" name="kgcm" placeholder="Quantité kg par lieu de stockage " />
												</div>
											</div>
										</div>
									</div>
